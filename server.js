...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    if (section === 'hero' && file.mimetype.startsWith('video/')) {
      ...
      cb(null, true);
    } else if (section === 'hero' && !file.mimetype.startsWith('video/')) {
      cb(new Error('Hero section only accepts video files'), false);
    } else if (section !== 'hero' && !file.mimetype.startsWith('image/')) {
      cb(new Error('Non-hero sections only accept image files'), false);
    } else {
      cb(null, true);
    }
  }
